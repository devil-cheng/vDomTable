...
const CustomTbody = forwardRef(({ columns, dataSource, height, scrollBar, rowHeight = 30 }, ref) => {
    ...
    const updateViewContentFn = () => {
      const clientHeight = bodyNode && bodyNode.clientHeight;
      // 计算可视区域里能放几个元素
    ...
    // 头部scroll监听
    const leftMouseFn = () => {
      bodyNode.removeEventListener("scroll", leftScrollFn);
        ...
        <div className={style.tbodyContainer} style={{height: height}}>
            {
                leftFixedColumns && leftFixedColumns.length !== 0 ? (
                    <div className={style.leftFixedTbody} style={{width: leftFixedWidth, bottom: scrollBar.scrollBarHeight }}>
                        <CustomBodyRow columns={leftFixedColumns} dataSource={dataList} height={height - scrollBar.scrollBarHeight} bodyHeight={bodyHeight} ref={leftFixedRef} />
                   </div>
               ) : null
            }
            <div className={style.tbody}>
                <CustomBodyRow columns={columns} dataSource={dataList} height={height} bodyHeight={bodyHeight} ref={ref} />
            </div>
        </div>
    )
})
